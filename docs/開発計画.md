...
Phase 12:  ルーム待機画面のポーリング対応 🔄  90%完了
```
...
### Phase 12: ルーム待機画面のポーリング対応 ⏱️ 3時間 🔄 **90%完了**
**目標**: 待機画面をポーリングで更新
...
- [x] チャット機能実装（バックエンド完了 ✅）
  ...
  - [x] RoomServiceにsendMessage/getMessagesメソッド追加
  - [x] POST /api/rooms/{roomId}/chat - メッセージ送信API実装
  - [x] GET /api/rooms/{roomId}/chat - メッセージ取得API実装（ページネーション対応）
  - [x] Swagger APIドキュメント更新
  - [ ] front/src/api/room.tsにチャットAPIクライアント実装（フロントエンド）
  - [ ] WaitingRoomPage.tsxにチャットUI実装（フロントエンド）
  - [ ] メッセージ表示とポーリング実装（フロントエンド）
  - [ ] チャット履歴表示（フロントエンド）
...
- ✅ チャットAPI完成（バックエンド）
- ⏳ チャットでコミュニケーション（フロントエンド実装後）
- ✅ 全員準備完了でゲーム開始可能

#### 残作業
- ✅ チャットAPI実装（バックエンド）← 完了！
- ⏳ チャットUI実装（フロントエンド）← 残り10%
