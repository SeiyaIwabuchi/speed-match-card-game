...
const GamePageContent: React.FC<GamePageProps> = ({ onNavigate }) => {
  ...
  const { gameState, loading, error, isPlayerTurn, fetchGameState, handlePlayCard, clearError } = useGame();
  ...
  const handleLeaveGame = () => {
    ...
    const handleCardClick = async (card: any, targetField: number) => {
    if (!isPlayerTurn()) {
    ...
    try {
      await handlePlayCard(card, targetField);
      // プレイ後に状態を再取得
